...
const GuestCount = ({ guestCount, updateBookingData, sessionId }) => {
  ...
  useEffect(() => {
    const fetchVenueCapacities = async () => {
      try {
        const response = await api.get('/api/admin/bookings/venues', { params: { sessionId } });
        const capacities = response.data.venues.map((venue) => venue.capacity);
      ...
      <div className="mt-12 p-5 bg-primary-50 border border-primary-100 rounded-lg flex items-center">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Our venues can accommodate from {minGuests} to {maxGuests} guests comfortably. We'll recommend
